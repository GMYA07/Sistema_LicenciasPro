...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-12">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-[#0C3B4C]">Nuevo Equipo - <?= $infoArea['nombreArea'] ?></h1>
            <p class="text-sm text-gray-500 font-medium mt-1">Creacion de nuevos equipos</p>
        </div>
    </div>

    <div class="flex flex-wrap items-center gap-3">
        <a href="<?= BASE_URL ?>/equipos/area?idArea=<?= $infoArea['idArea']; ?>" class="flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-600 border border-gray-200 px-5 py-2.5 rounded-xl font-medium text-sm shadow-sm transition-all duration-300">
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm w-full max-w-3xl mx-auto overflow-hidden flex flex-col mt-6">
    ...
    <form action="<?= BASE_URL ?>/equipos/area/create" method="POST" class="p-8 flex flex-col gap-6">
        <div>
            ...
            <div class="relative">
                <select name="idAreaComputadora" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-700 focus:outline-none focus:border-[#2CA1C8] focus:ring-1 focus:ring-[#2CA1C8] focus:bg-white transition-all appearance-none">
                    <option value="<?= $infoArea['idArea']; ?>" selected><?= htmlspecialchars($infoArea['nombreArea']); ?></option>
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="Modelo" required placeholder="Ej: OptiPlex 3080" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#2CA1C8] focus:ring-1 focus:ring-[#2CA1C8] focus:bg-white transition-all">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="Serial" required placeholder="Ej: SN-987654321" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#2CA1C8] focus:ring-1 focus:ring-[#2CA1C8] focus:bg-white transition-all">
            </div>
